...
#include "usb.h"
#include "mx28_pins.h"
//modefy by zhuguojun  PINID_AUART2_TX在283是usb power控制但在287是作串口用，而在287、280usb power是不用的
//所以为了防止这里代码干扰串口，把USB_POWER_ENABLE接去led处
#if defined(CONFIG_iMX_287) 
#define USB_POWER_ENABLE MXS_PIN_TO_GPIO(PINID_LCD_D22)
#endif

#if  defined(CONFIG_iMX_280)
#define USB_POWER_ENABLE MXS_PIN_TO_GPIO(PINID_SSP0_DATA7)
#endif

#if defined(CONFIG_iMX_283)			//这是283
