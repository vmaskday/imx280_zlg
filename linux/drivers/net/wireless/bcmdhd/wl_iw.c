...
wl_iw_iscan_get(iscan_info_t *iscan)
{
	...
	wl_update_bss_cache(&g_bss_cache_ctrl, results);
#endif
#if defined(RSSIAVG)
	wl_delete_dirty_rssi_cache(&g_rssi_cache_ctrl);
	wl_update_rssi_cache(&g_rssi_cache_ctrl, results);
#endif
	DHD_OS_MUTEX_UNLOCK(&wl_cache_lock);
	return status;
}
...
#endif
#if defined(RSSIOFFSET)
		bi->RSSI = htod16(wl_update_rssi_offset(dtoh16(bi->RSSI)));
#endif
...
#if defined(BSSCACHE)
static int
wl_iw_merge_bss_cache(struct iw_request_info *info, char *extra,
	__u16 buflen_from_user, __u16 *merged_len, uint32 *apcnt)
{
	wl_bss_cache_t *node;
	wl_scan_results_t *list_merge;
	...
	for (;node;) {
		list_merge = (wl_scan_results_t *)&node->results;
		if (buflen_from_user - *merged_len > 0) {
			len_prep = 0;
			ret = wl_iw_get_scan_prep(list_merge, info, extra+*merged_len,
				buflen_from_user-*merged_len, &len_prep, apcnt, true);
			if (ret < 0)
				break;
			*merged_len += len_prep;
		} else {
...
)
{
	...
	if (iscan->iscan_state == ISCAN_STATE_SCANING) { // terence: if scaning, don't reply.
...
#if defined(BSSCACHE)
	ret = wl_iw_merge_bss_cache(info, extra+len_ret, buflen_from_user-len_ret,
		&len_ret, &apcnt);
	if (ret < 0)
		return ret;
	dwrq->length = len_ret;
...
)
{
	...
	if (!(ret = dev_wlc_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(channel_info_t)))){
		WL_TRACE(("hw_channel %d ",ci.hw_channel));
		WL_TRACE(("target_channel %d ",ci.target_channel));
		WL_TRACE(("scan_channel %d\n",ci.scan_channel));
		extra += sprintf(extra, "channel %d ", ci.hw_channel);
	}
...
)
{
	...
	if (g_wifi_on == G_WLAN_SET_ON) {
		sscanf(extra, "%*s %10d", &roam_trigger[0]);
		roam_trigger[1] = WLC_BAND_ALL;
		if ((error = dev_wlc_ioctl(dev, WLC_SET_ROAM_TRIGGER, roam_trigger,
				sizeof(roam_trigger))) >= 0) {
			p += snprintf(p, MAX_WX_STRING, "OK");
			WL_TRACE(("%s: WLC_SET_ROAM_TRIGGER %d OK\n", __FUNCTION__, roam_trigger[0]));
			goto exit;
		} else {
			WL_ERROR(("%s: WLC_SET_ROAM_TRIGGER %d failed code %d\n", __FUNCTION__,
				roam_trigger[0], error));
		}
	}
	p += snprintf(p, MAX_WX_STRING, "FAIL");
exit:
...
)
{
	...
	if (g_wifi_on == G_WLAN_SET_ON) {
		...
		if (!(dev_wlc_ioctl(dev, WLC_GET_ROAM_TRIGGER, roam_trigger, sizeof(roam_trigger)))) {
			trigger[0] = roam_trigger[0];
		} else
			WL_ERROR(("%s: 2G WLC_GET_ROAM_TRIGGER %d failed code %d\n",
				__FUNCTION__, roam_trigger[0], error));

		roam_trigger[1] = WLC_BAND_5G;
		if (!(dev_wlc_ioctl(dev, WLC_GET_ROAM_TRIGGER, roam_trigger, sizeof(roam_trigger)))) {
			trigger[1] = roam_trigger[0];
		} else
			WL_ERROR(("%s: 5G WLC_GET_ROAM_TRIGGER %d failed code %d\n",
				__FUNCTION__, roam_trigger[0], error));

	}
	p += snprintf(p, MAX_WX_STRING, "roam_trigger %d %d", trigger[0], trigger[1]);
...
)
{
	...
	WL_TRACE(("%s: command = %s\n", __FUNCTION__, extra));

	buf = kmalloc(WLC_IOCTL_MAXLEN, GFP_KERNEL);
	sscanf(extra+strlen(SET_KEEP_ALIVE), "%d", &period_msec);
	...
	if ((res = wldev_ioctl(dev, WLC_SET_VAR, buf, buf_len, TRUE)) != 0)
        WL_ERROR(("%s: mkeep_alive set failed. res[%d]\n", __FUNCTION__, res));
...
)
{
	...
	if ((res = wldev_ioctl(dev, WLC_GET_VAR, mkeep_alive_pktp, len, FALSE))<0) {
		WL_ERROR(("%s: GET mkeep_alive ERROR %d\n", __FUNCTION__, res));
		goto exit;
	} else {
		printf("Id            :%d\n"
			   "Period (msec) :%d\n"
	...
	extra += sprintf(extra, "mkeep_alive_period_msec %d ",  dtoh32(mkeep_alive_pktp->period_msec)) + 1;
	extra += sprintf(extra, "0x");
	for (i=0; i<mkeep_alive_pktp->len_bytes; i++) {
		extra += sprintf(extra, "%x", mkeep_alive_pktp->data[i]);
	}
...
)
{
	...
	strncpy((char *)&pm, extra + strlen(SET_PM)+1, 1);

	if (!(ret = dev_wlc_ioctl(dev, WLC_SET_PM, &pm, sizeof(int)))){
        WL_ERROR(("%s: WLC_SET_PM set failed. ret=%d\n", __FUNCTION__, ret));
	}
...
)
{
	...
	if (!(ret = dev_wlc_ioctl(dev, WLC_GET_PM, &pm, sizeof(int)))) {
		WL_TRACE(("PM %d\n", pm));
		extra += sprintf(extra, "PM %d", pm);
	}
...
#endif
		else if(strnicmp(extra, GET_CUR_CHANNEL, strlen(GET_CUR_CHANNEL)) == 0)
			ret = wl_iw_get_channel(dev, info, (union iwreq_data *)dwrq, extra);
		else if(strnicmp(extra, SET_ROAM_TRIGGER, strlen(SET_ROAM_TRIGGER)) == 0)
			ret = wl_iw_set_roam_trigger(dev, info, (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, GET_ROAM_TRIGGER, strlen(GET_ROAM_TRIGGER)) == 0)
			ret = wl_iw_get_roam_trigger(dev, info, (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, SET_KEEP_ALIVE, strlen(SET_KEEP_ALIVE)) == 0)
			ret = wl_iw_set_keep_alive(dev, info, (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, GET_KEEP_ALIVE, strlen(GET_KEEP_ALIVE)) == 0)
			ret = wl_iw_get_keep_alive(dev, info, (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, SET_PM, strlen(SET_PM)) == 0)
			ret = wl_iw_set_pm(dev, info, (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, GET_PM, strlen(GET_PM)) == 0)
			ret = wl_iw_get_pm(dev, info, (union iwreq_data *)dwrq, extra);
	    else {
...
wl_iw_bt_release(void)
{
	...
	del_timer_sync(&bt_local->timer); // 20120501: fix for some platform can not kill timer in thread
	kfree(bt_local);
	g_bt = NULL;
}
